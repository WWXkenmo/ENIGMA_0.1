...
#' @export
#'
batch_correct <- function(object, varname_cell_type, n_pseudo_bulk=100, sample_size = 1000) {
    if (object@ref_type == "single_cell") {
        message(date(), " Reference is from Single Cell RNA-seq, doing batch correction in S mode. ")
        object = remove_batch_effect_S_mode(object, varname_cell_type = varname_cell_type, n_pseudo_bulk=n_pseudo_bulk,sample_size = sample_size)
    }
