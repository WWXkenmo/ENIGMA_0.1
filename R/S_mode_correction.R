...
#'
remove_batch_effect_S_mode <- function(object, varname_cell_type, n_pseudo_bulk, sample_size = sample_size) {
    bulk_matrix = assay( object@raw_input$bulk, "raw" )
    ref_se = object@raw_input$ref

    cat(date(), "Generating pseudo bulk... \n")
    pseudo_bulk_main_ct = generate_pseudo_bulk_from_scRNA( ref_se, varname_cell_type, n = n_pseudo_bulk, sample_size = sample_size)
...
generate_pseudo_bulk_from_scRNA <- function(ref_se, ct_varname, n, sample_size) {
    ...
    for(i in 1:nrow(frac)){
        Ma = lapply( meta_ref[,ct_varname] %>% unique, function(ct) {
            sample_ids = subset( meta_ref, eval(parse( text = paste0(ct_varname, "==\"", ct, "\"") )) ) %>%
                rownames %>%
                sample( sample_size*frac[i,ct], replace = TRUE )
